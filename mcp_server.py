#!/usr/bin/env python3
"""Social MCP Server - Unified Douyin & Xiaohongshu"""
...
def _xhs_parse(note):
    ...
    for img in (note.get("imageList") or []):
        ...
        if u: imgs.append(u)
    it=note.get("interactInfo") or {}; user=note.get("user") or {}; vu=None
    v=note.get("video")or{};s=(v.get("media")or{}).get("stream")or{}
    for c in ("h264","h265","h266","av1"):
        for item in (s.get(c)or[]):
            if item.get("masterUrl"): vu=item["masterUrl"]; break
        if vu: break
    return {"title":(note.get("title")or"").strip(),"desc":(note.get("desc")or"").strip().replace("\xa0"," "),
            ...
            "images":imgs,"image_count":len(imgs),"video_url":vu,
            "note_type":"video"if(note.get("type")=="video"or vu)else"image","platform":"xiaohongshu"}
...
def parse_xiaohongshu_note(url):
    ...
    if"请通过小红书"in html or"verify"in fu.lower(): return {"status":"error","error":"被风控拦截","platform":"xiaohongshu"}
    st=_xhs_state(html); data=None
    if st:
        n=_xhs_note(st)
        if n: data=_xhs_parse(n)
    if not data or not(data.get("desc")or data.get("title")):
        def m(p):
            x=re.search(rf'<meta[^>]+(?:property|name)=["\']{re.escape(p)}["\'][^>]+content=["\'](.*?)["\']',html,re.I)
            return x.group(1).strip() if x else ""
        t,d=m("og:title"),m("og:description")
    ...
    if not data or not(data.get("desc")or data.get("title")): return{"status":"error","error":"无法解析笔记内容","platform":"xiaohongshu"}
...
def _call_vlm_video_url(video_url,prompt,ak):
    ...
    r.raise_for_status();return r.json()["output"]["choices"][0]["message"]["content"]

def _call_vlm_video(frames,prompt,ak,fps=2.0):
    h={"Authorization":f"Bearer {ak}","Content-Type":"application/json"}
    p={"model":DASHSCOPE_VISION_MODEL,"input":{"messages":[{"role":"user","content":[{"video":frames,"fps":fps},{"text":prompt}]}]},"parameters":{"max_tokens":4096}}
    r=requests.post(DASHSCOPE_MULTIMODAL_URL,headers=h,json=p,timeout=120)
    r.raise_for_status();return r.json()["output"]["choices"][0]["message"]["content"]
...
def parse_share_link(share_link:str)->str:
    ...
    try:
        ...
        if x.get("status")=="success"and(x.get("title")or x.get("desc")):
            x["platform_detected_by"]="xiaohongshu (fallback)";return json.dumps(x,ensure_ascii=False,indent=2)
...
def analyze_share_images(share_link:str)->str:
    ...
    try:
        ...
        if p=="xiaohongshu":
            ...
            info=_douyin_rich_parse(share_link);us=list(info.get("images",[]))
...
def analyze_share_video(share_link:str,num_frames:int=8)->str:
    ...
    try:
        ...
        try:
            ...
            b64s=[_img_b64(f)for f in frames]
            ds=subprocess.check_output([_find_bin("ffprobe"),"-v","error","-show_entries","format=duration","-of","default=noprint_wrappers=1:nokey=1",str(vp)],timeout=15).decode().strip()
            dur=float(ds)if ds else 30
